...
$b = $_GET['b'];
    if($b == "import" && isset($_FILES['json_file'])) {
        ...
        if(is_array($data)) {
            ...
            if(file_put_contents("backend/languages/$id.php", $contents)) {
    ...
    elseif($b == "export") {
        $id = $_GET['id'];
        $filePath = "backend/languages/$id.php";
    ...
    elseif($b == "translate_api") {
        ...
        if(file_exists($filePath)) {
            ...
            if (isset($lang) && is_array($lang)) {
                ...
                $tempJson = "backend/languages/temp_trans_" . time() . ".json";
                ...
                curl_setopt_array($curl, [
                    ...
                    CURLOPT_FOLLOWLOCATION => true, // اتباع أي تحويلات للرابط
                    CURLOPT_POST => true,
                    CURLOPT_HTTPHEADER => [
                        'X-API-Key: ' . $apiKey,
                ...
                $httpCode = curl_getinfo($curl, CURLINFO_HTTP_CODE); // كود حالة الاستجابة
...
if($b == "set_default") {
	$id = $_GET['id'];
	if(!file_exists("backend/languages/$id.php")) {
...
';
						...
						$contents .= '?>';
						$update = file_put_contents("backend/languages/$lang_name.php",$contents);
						if($update) {
					...
					<form action="" method="POST">
						...
						<table class="table table-bordered">
							...
							<tbody>
							<?php
							include("backend/languages/en.php");
							foreach($lang as $k=>$v) {
								echo '<tr>
										<td><b>'.$k.'</b></td>
...
';
						...
						$contents .= '?>';
						$update = file_put_contents("backend/languages/$id.php",$contents);
						if($update) {
					...
					<form action="" method="POST">
						<table class="table table-bordered">
							...
							<tbody>
							<?php
							include("backend/languages/$id.php");
							foreach($lang as $k=>$v) {
								echo '<tr>
										<td><b>'.$k.'</b></td>
										<td><input type="text" class="form-controL" name="key['.$k.']" value="'.$v.'"></td>
	...
	?>
           ...
           <div class="col-md-12">
			...
			if("en" == $id) { 
				...
				@unlink("backend/languages/$id.php");
...
?>
		...
		<table class="table table-striped">
			...
			<tbody>
				<?php
				if ($handle = opendir('backend/languages')) {
